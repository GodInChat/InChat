...
POSTGRES_VECTOR_DB = settings.postgres_vector_db
...
try:
    conn = psycopg2.connect(database=POSTGRES_VECTOR_DB,
                            user=POSTGRES_USER,
...
#except psycopg2.OperationalError as e:
    ...
    if errmsg.find("does not exist") >= 0:
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(f'CREATE DATABASE "{POSTGRES_VECTOR_DB}"')
            print(f"[OK] DATABASE {POSTGRES_VECTOR_DB} HAS BEEN CREATED SUCCESSFULLY");
            exit(exitcode)
        except psycopg2.Error as ee:
            print(f"Error {ee.pgcode}: Create database {POSTGRES_VECTOR_DB}: {str(ee)}", end="")
            exitcode=101
...
print(f"[OK] DATABASE {POSTGRES_VECTOR_DB} ALREADY EXISTS")
